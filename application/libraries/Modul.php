...
class Modul {
    ...
    public function BulanTtp() {
        date_default_timezone_set("Asia/Jakarta");
        return date(" M ");
    }

    public function TahunTtp() {
        date_default_timezone_set("Asia/Jakarta");
        return date(" Y ");
    }
    ...
    public function autokode_mutasi($depan, $kolom, $table, $awal, $akhir, $lokasi) {
        $hasil = "";
        $q_data = mysqli_query($this->getkoneksi(), "select ifnull(MAX(substr(".$kolom.",".$awal.",".$akhir.")),0) + 1 as jml from ".$table." where SUBSTRING('".$lokasi."', 4,1) = 'T' ;");
        $data_query = mysqli_fetch_array($q_data);
        $panjang = strlen($data_query['jml']);
        $pnjng_nol = ($akhir-$panjang) - $awal;
        $nol = "";
    ...
    public function autokodecutomer($depan, $kolom, $table, $awal, $akhir, $lokasi, $tipe) {
        $hasil = "";
        $q_data = mysqli_query($this->getkoneksi(), "select ifnull(MAX(substr(".$kolom.",".$awal.",".$akhir.")),0) + 1 as jml from ".$table." where idlokasi_customer = '".$lokasi."' and idtipe_customer = '".$tipe."';");
        $data_query = mysqli_fetch_array($q_data);
        $panjang = strlen($data_query['jml']);
        $pnjng_nol = ($akhir-$panjang) - $awal;
        $nol = "";
    ...
    public function autokode3($depan, $kolom, $table, $awal, $akhir, $cabang) {
        $hasil = "";
        $q_data = mysqli_query($this->getkoneksi(), "select ifnull(MAX(substr(".$kolom.",".$awal.",".$akhir.")),0) + 1 as jml from ".$table." where iddivisi = '".$cabang."';");
        $data_query = mysqli_fetch_array($q_data);
        $panjang = strlen($data_query['jml']);
        $pnjng_nol = ($akhir-$panjang) - $awal;
        $nol = "";
