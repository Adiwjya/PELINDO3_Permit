<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $('#tb').DataTable( {
			ajax: "<?php echo base_url(); ?>rip/ajax_list",
			aaSorting: [[2, 'desc']]
		});
	...
	function tambah(){
        window.location.href = "<?php echo base_url(); ?>rip/new_add";
    }
	
	function ganti(id){
        window.location.href = "<?php echo base_url(); ?>rip/new_add/"+id;
    }
	...
	function response(id) {
		...
		// alert(id);
		$.ajax({
            url : "<?php echo base_url(); ?>rip/load_response/" + id,
            type: "POST",
            dataType: "JSON",
	...
	function hapus(id, nama){
        if(confirm("Apakah anda yakin menghapus " + nama + " ?")){
            $.ajax({
                url : "<?php echo base_url(); ?>rip/hapus/" + id,
                type: "POST",
                dataType: "JSON",
				data: $('#f_csrf').serialize(),
                success: function(data){
