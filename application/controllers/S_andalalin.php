...
class S_andalalin extends CI_Controller {
	...
	public function ajax_list() {
        if (get_cookie('status') == "login") {
			$data = array();
            $list = $this->Mglobals->getAllQ("select * from REKOM_ANDALALIN where DELETE_STATUS = '0'");
	...
	public function new_add()
	{
		if (get_cookie('status') == "login") {
			...
			if ($kond == null) {
				...
				$data_edit = $this->Mglobals->getAllQR("select * from REKOM_ANDALALIN where ID_PENGAJUAN ='".$kond."' ");
				$data['jenis_perizinan'] = $this->Mglobals->getAll("JENIS_IZIN");
				...
				if (0 < $_FILES['file']['error']) {
					...
					if ($this->upload->do_upload('file')) {
						...
						if ($this->input->post('id_izin') == "") {
							...
							$q_data = $this->Mglobals->getAllQR("select NVL(MAX(substr(ID_PENGAJUAN,'4','7')),0) + 1 as jml from REKOM_ANDALALIN ");
							// var_dump($nilai);
							$data_input = array(
								'ID_PENGAJUAN' => $this->modul->autokode_oci('RAL','4','7',$q_data->JML), //Auto kode OCI
							...
							$simpan  = $this->Mglobals->add('REKOM_ANDALALIN', $data_input);
						}else{
							// Update data
							$data_input = array(
								'JUDUL_PERIZINAN' => $this->input->post('judul'),
							...
							$simpan  = $this->Mglobals->update("REKOM_ANDALALIN", $data_input, $condition);
						}
	...
	//                    
		...
		public function hapus() {
			if (get_cookie('status') == "login") {
				...
				$hapus = $this->Mglobals->update("REKOM_ANDALALIN",$data,$kond);
				if($hapus == 1){
