...
class I_reklamasi extends CI_Controller {
	...
	public function ajax_list() {
        if (get_cookie('status') == "login") {
			$data = array();
            $list = $this->Mglobals->getAllQ("select * from PENGAJUAN_IZIN_REKLAMASI where DELETE_STATUS = '0'");
				...
				if (0 < $_FILES['file']['error']) {
					...
					if ($this->upload->do_upload('file')) {
						...
						if ($this->input->post('id_izin') == "") {
							...
							$q_data = $this->Mglobals->getAllQR("select NVL(MAX(substr(ID_PENGAJUAN,'4','7')),0) + 1 as jml from PENGAJUAN_IZIN_REKLAMASI ");
							// var_dump($nilai);
							$data_input = array(
								'ID_PENGAJUAN' => $this->modul->autokode_oci('IRL','4','7',$q_data->JML), //Auto kode OCI
