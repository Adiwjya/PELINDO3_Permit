...
class Rip extends CI_Controller {
	...
	public function ajax_list() {
        if (get_cookie('status') == "login") {
			$data = array();
            $list = $this->Mglobals->getAllQ("select * from PENGAJUAN_IZIN_RIP where DELETE_STATUS = '0'");
	...
	public function new_add()
	{
		if (get_cookie('status') == "login") {
			...
			if ($kond == null) {
				...
				$this->load->view('menu');
				$this->load->view('rip/add');
				$this->load->view('fitur');
				$this->load->view('footer');
			}else {
				$data_edit = $this->Mglobals->getAllQR("select * from PENGAJUAN_IZIN_RIP where ID_PENGAJUAN ='".$kond."' ");
				$data['jenis_perizinan'] = $this->Mglobals->getAll("JENIS_IZIN");
				...
				if (0 < $_FILES['file']['error']) {
					...
					if ($this->upload->do_upload('file')) {
						...
						if ($this->input->post('id_izin') == "") {
							...
							$q_data = $this->Mglobals->getAllQR("select NVL(MAX(substr(ID_PENGAJUAN,'4','7')),0) + 1 as jml from PENGAJUAN_IZIN_RIP ");
							// var_dump($nilai);
							$data_input = array(
								'ID_PENGAJUAN' => $this->modul->autokode_oci('RIP','4','7',$q_data->JML), //Auto kode OCI
							...
							$simpan  = $this->Mglobals->add('PENGAJUAN_IZIN_RIP', $data_input);
						}else{
							// Update data
							$data_input = array(
								'JUDUL_PERIZINAN' => $this->input->post('judul'),
							...
							$simpan  = $this->Mglobals->update("PENGAJUAN_IZIN_RIP", $data_input, $condition);
						}
	...
	//                    
		...
		public function hapus() {
			if (get_cookie('status') == "login") {
				...
				$hapus = $this->Mglobals->update("PENGAJUAN_IZIN_RIP",$data,$kond);
				if($hapus == 1){
