...
class Rip extends CI_Controller {
	...
	public function ajax_list() {
        if (get_cookie('status') == "login") {
			$data = array();
            $list = $this->Mglobals->getAllQ("select * from PENGAJUAN_RIP where DELETE_STATUS = '0'");
	...
	public function new_add()
	{
		if (get_cookie('status') == "login") {
			...
			if ($kond == null) {
				...
				$data_edit = $this->Mglobals->getAllQR("select * from PENGAJUAN_RIP where ID_PENGAJUAN ='".$kond."' ");
				$data['jenis_perizinan'] = $this->Mglobals->getAll("JENIS_IZIN");
				...
				if (0 < $_FILES['file']['error']) {
					...
					if ($this->upload->do_upload('file')) {
						...
						if ($this->input->post('id_izin') == "") {
							...
							$q_data = $this->Mglobals->getAllQR("select NVL(MAX(substr(ID_PENGAJUAN,'3','7')),0) + 1 as jml from PENGAJUAN_RIP ");
							// var_dump($nilai);
							$data_input = array(
							...
							$simpan  = $this->Mglobals->add('PENGAJUAN_RIP', $data_input);
						}else{
							// Update data
							$data_input = array(
								'JUDUL_PERIZINAN' => $this->input->post('judul'),
							...
							$simpan  = $this->Mglobals->update("PENGAJUAN_RIP", $data_input, $condition);
						}
	...
	//                    
		...
		public function hapus() {
			if (get_cookie('status') == "login") {
				...
				// $hapus = $this->Mglobals->delete("PENGAJUAN_RIP", $kond);
				// if($hapus == 1){
				...
				$hapus = $this->Mglobals->update("PENGAJUAN_RIP",$data,$kond);
				if($hapus == 1){
					$status['message'] = "Data terhapus";
				}else{
					$status['message'] = "Data gagal terhapus";
